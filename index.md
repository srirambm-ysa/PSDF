...
Never merge AI-generated code you cannot explain line by line.
Every AI session starts with a context file. No context file = no AI session.
Tests are written before AI generates code. Always.
Acceptance criteria from BA is the only source of truth. Not AI output.
...
Note the failure in the relevant CONTEXT/[feature].md under "Known AI failure points"
Log it in the sprint retrospective
If it's a recurring hallucination on a specific library/pattern — add a constraint to the prompt template
...
Traditional: Defines what "done" means
PSDF-specific: Becomes the persistent context seed for all AI sessions in that feature
...
What did AI generate correctly this sprint?
What did AI hallucinate or get wrong?
Which prompts worked reliably? Add to prompt library.
Which context files need updating?
Is the team's mental model shifting from deterministic to probabilistic?
...
For explaining and selling PSDF to MSME clients
...
Owner	[Team Lead / Organisation name]
